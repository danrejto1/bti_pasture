...
meat_cons_global <- read_csv(file =  "../data/clean_data/meat_cons_global.csv")
milk_cons_global <- read_csv(file =  "../data/clean_data/milk_cons_global.csv")

meat_prod_global <- read_csv(file =  "../data/clean_data/meat_prod_global.csv")
milk_prod_global <- read_csv(file =  "../data/clean_data/milk_prod_global.csv")

meat_yields_global <- read_csv(file =   "../data/clean_data/meat_yields_global.csv")
milk_yields_global <- read_csv(file =  "../data/clean_data/milk_yields_globa
                               l.csv")

stocks_global <- read_csv(file =  "../data/clean_data/stocks_global.csv")
livestock_density_global <- read_csv(file =   "../data/clean_data/livestock_density_global.csv")
major_crosswalk <- read_csv(file =   "../data/clean_data/major_crosswalk.csv")
minor_crosswalk <- read_csv(file =   "../data/clean_data/minor_crosswalk.csv")
hyde <- read_csv(file =   "../data/clean_data/hyde.csv")
combined_country_change <- read_csv(file =   "../data/clean_data/combined_country_change.csv")
...
pasture_projections <- read_csv(file="../data/pasture_projections/pasture_projections_for_graphing.csv")
```
...
combined_country <- combined_country %>%
  filter(country!="Eritrea",                #Eritrea became independent from Ethiopia in 1993 and... 
         !str_detect(country, "Ethiopia"),  #Ethiopia's pasture area nearly dropped in half by~18 Mha (even when including Eritrea area)
         !str_detect(country, "Iran"), #Omit Iran since its pasture suddenly drops ~17 Mha without explanation in 2005.
         inc_group_16 %in% c("H", "UM", "LM", "L"), #omit small countries w/o income for plotting
...
#combined_country[combined_country$country =="Saudi Arabia",]$group <- "Saudi Arabia"
...
# Section A: Global Increases in Livestock Productivity Have Reduced Pasture Area
## After decades or centuries of growth, global pasture area has recently plateaued and even begun decreasing

```{r Line Graph of global pasture area since 1961}
...
pasture_global_pct_fall <- (1-(pasture_global_latest/pasture_global_2000))*100    #percent difference between 2000 and latest year

pasture_global_abs_fall <- pasture_global_2000 - pasture_global_latest   #absolute difference from 2000 to latest year

#avg. annual rate of pasture change in 16 years since 2000
pasture_change_rate_post2000 <- -pasture_global_pct_fall/16
...
p <- ggplot(pasture_global) +
  ...
  labs(title = paste0("Fig 1: Global Pasture Area Declined ", round(pasture_global_pct_fall,1), "% (", round(pasture_global_abs_fall/1000000,1), " Mha) since 2000"),  
...
- This contraction in pasture is corroborated by multiple models (HYDE Graph in Appendix)
- While cropland area for feed crops has grown, this growth is small enough that the trend in pasture + feed cropland is similar to the pasture trend (Appendix graph  of the number of cattle globally per unit of agricultural land (combining pasture and cropland) 
...
countries_up_then_down <- intersect(countries_up_61_00,countries_down_00_16)
...
#p_area_down_00_16
```
`r p_countries_down_or_flat_00_16`% of countries reported a net pasture decline between 2000 and 2016.
 
 
 
## The decline in pasture has not been driven by changes in consumption, but rather by increases in livestock productivity
- Growth in consumption of ruminant products has slowed, contributing to the decline in pasture area.
...
meat_prod_change_2000 <- pct_chg(meat_prod_global, "production", 2000, 2013)

# Production of buffalo, cattle, goat, sheep MILK
milk_prod_change_2000 <- pct_chg(milk_prod_global, "production", 2000, 2013)
...
# Production of buffalo, cattle, goat, sheep MEAT 
meat_prod_change_1961 <- pct_chg(meat_prod_global, "production", 1961, 2013)

# Production of buffalo, cattle, goat, sheep MILK
milk_prod_change_1961 <- pct_chg(milk_prod_global, "production", 1961, 2013)
...
p <- decadal_cons_yld_changes %>% 
  ...
  labs(title = "Fig 2a) Global Meat Consumption Growth Has Slowed While Dairy Has Not",
       y = paste0("Annual Percent Change"),
       x = "Period")+
    theme_hc() +
    facet_wrap(Product~.) #uncomment and delete above line to show all vars

p
ggsave(filename = "../outputs/decadal_change_consumption.pdf", plot = p, width = 8, height=8)
```
...
```{r Line Graph of index of global pasture, ruminant meat production, and ruminant milk production}
base_year = 1961
global_indices <- combined_country %>%
...
p <- global_indices %>%
  ...
  ggplot()+
    geom_line(aes(x=year, y=(value-1)*100, color=Variable))+
  scale_x_continuous(breaks = since_1961, limits = c(base_year, 2013)) +
  labs(title = "Fig 2b) Global Beef and Milk Production Increased While Pasture Area Declined",
...
- As pasture area has declined from 2000 to 2016, production of cattle meat and milk products has increased `r round(meat_prod_change_2000$percent_difference, 1)`% and `r round(milk_prod_change_2000$percent_difference, 1)`%, respectively. 

## The livestock revolution - shifting from extensive to intensive production - has led to large productivity gains that enabled this decoupling. 

- Productivity is comprised of slaughter weight, dressing percentage, stocking rate, and the portion of animals raised slaughtered each year (combining turnover with metrics of animal health). Feed use efficiency is another productivity metric
- Sidebar/box with productivity definitions.

## Meat and milk productivity have increased in all major livestock production regions
```{r Global meat and milk productivity changes}
# p <- global_indices %>%
...
#     geom_line(aes(x=year, y=(value-1)*100, color=Variable))+
#     scale_x_continuous(breaks = since_1961) +
#     labs(title = "Fig 3) Global Beef and Milk Productivity Increases Key to Pasture Decline",
#          y = "Percent Change since 1961",
#          x = "Year") +
#     scale_color_discrete(name = element_blank(),
#                         breaks=c("pct_chg_m_yld", "pct_chg_d_yld", "pct_chg_stocking"),
#                          labels=c("Beef Yields", "Milk Yields", "Cattle Stocks per Unit Pasture")) +
#     theme_hc()
# p
```


```{r Calculate change in  meat & milk yields and stocking by group}
# set baseline year
baseline_year = 1961

regional_indices <- combined_country %>%
  filter(group !="Former Soviet Union", #omits former soviet union becuase of data consistency problems
         !str_detect(country, "Hong Kong"),# omit hong kong which is an outlier in yields
         year >= baseline_year) %>%   #limit to years after baseline
  filter(group !="Saudi Arabia") %>% #omits Saudi Arabia becuase of data consistency problems especially in 1970s and early 80s
  group_by(year, group) %>%
...
```{r Graph change in  meat & milk yields and stocking by group}
...
p <- regional_indices %>%
  ...
  ggplot(aes(x= reorder(group, -value), y=value-1))+
    geom_col(aes(fill=variable, group=variable), position="dodge") +
    labs(title="Fig 3: Cattle Productivity Increased in Every Region",
         x=element_blank(),
         y="Percent Increase Since 1961",
         caption = "Omits Saudi Arabia, Former Soviet Union and Hong Kong due to data quality issues") +
...
# +
#     facet_wrap(variable~., ncol = 1, strip.position = "right", dir = )
...
# p <- regional_indices %>% 
#   filter(group !="Saudi Arabia") %>% #omits Saudi Arabia becuase of data consistency problems especially in 1970s and waerly 80s
...
- The number of animals per area of pasture has increased as well, reflecting the shift to more intensive grazing, greater animal confinement, and other forms of intensification.



## Pasture area shrank most where the rate of productivity growth was high relative to production growth, and expanded in areas where relative productivity growth was low.
- Since 1961, Pasture area declined most in countries that are high-income today and expanded most in countries that are upper-middle income today
```{r Pasture Area Change since 1961 in Mha by Income Level}
...
#omits NA's - currently Czechoslovakia, Falkland Islands (Malvinas), French Guiana, Guadeloupe, Martinique, Mayotte, Montserrat, Niue, Norfolk Island, Pacific Islands Trust Territory, Réunion, Saint Helena 
...
p <- ggplot(pasture_area_change_by_income) +
  ...
  labs(x="Year", y="Pasture Area (Mha)", 
       title = "Fig )Pasture Rose for Upper-Middle & Fell for High Income Countries Since 1961", 
       caption = "Omits Former Soviet Union & Countries Missing World Bank Income Group") +
  scale_x_continuous(breaks = since_1961) + 
...
income_indices <- combined_country %>%
  filter(group !="Former Soviet Union", #omits former soviet union becuase of data consistency problems
         !str_detect(country, "Hong Kong"),# omit hong kong which is an outlier in yields
        country != "Equatorial Guinea",  #omit b/c of outlier gdp
         country != "Republic of Korea", #omit b/c of outlier gdp
         year >= baseline_year) %>%   #limit to years after baseline
  filter(group !="Saudi Arabia") %>% #omits Saudi Arabia becuase of data consistency problems especially in 1970s and early 80s
  group_by(year, inc_group_16) %>%
...
income_indices <- income_indices %>% 
  ...
  filter(inc_group_16 !="..", is.na(inc_group_16)==F) #omits countries w/o income group - e.g. British Virgin Islands
...
```{r Map of pasture area change by country since 1961 (in Mha)}
...
```{r Map of avg. annual pasture area change 2000-2016}
pasture_chg_annual_00_16 <- combined_country %>%
  ...
  dplyr::group_by(country, iso_code) %>% 
  dplyr::summarize(value = (sum(abs_chg_area, na.rm=T)/1000000)/(2016-2000)) %>% #avg. annual change in Mha
  filter(is.na(value)==F)
...
mapParams <- mapCountryData(viz, 
                            nameColumnToPlot="value", 
                            mapTitle="Fig : Average Annual Change in Pasture Area 2000-2016 (Million ha)", 
                            numCats = 30, #number of bins 
                            catMethod = c(-6,-5,-4,-3,-2,-1,-.5,-.0001,.0001,.5,1,2,3,4,5,6),             
                            colourPalette =   "diverging", #c("dark green", "green", "white", "orange", "red"),
                            oceanCol = "aliceblue",
...
- These pasture increases, at the most fundamental level, were driven by a a high rise in production (driven by domestic or global demand) relative to the rise in productivity. Areas where pasture contracted had a lower relative increases in production.
...
plot_dat <- combined_country %>%
  filter(group !="Former Soviet Union") %>% #omits soviet union
  group_by(year, group) %>%
  summarize(tot = sum(area, na.rm = T)) 
...
## Without these productivity gains, more pasture land would be needed
...
m_prod_per_ha_2013 <- combined_country %>% filter(year==2013) %>% group_by(year) %>% summarize(tot = sum(d_prod, na.rm=T)/sum(area, na.rm=T))%>% select(tot)
...
d_production_2013 <- combined_country %>% filter(year==2013) %>% group_by(year) %>% summarize(tot=sum(m_prod, na.rm=T)) %>% select(tot)
m_extra_land <- (m_production_2013 / m_prod_per_ha_1961) - (m_production_2013 / m_prod_per_ha_2013)
d_extra_land <- (d_production_2013 / d_prod_per_ha_1961) - (d_production_2013 / d_prod_per_ha_2013)
...
- For every 10% increase in beef and dairy yields, pasture area generally declines X-Y% (Appendix of regression analyses). 




# Section B: Challenges Ahead
## If pasture productivity does not keep pace with demand growth, global pasture area could start to expand once again

## Forecasts predict that the recent increases in meat and milk demand in lower income countries will continue, as well as the declines in demand in high income countries

## With production shifting to lower productivity countries, productivity may not keep pace with demand. 
...
```{r Yield Growht in High income countries by decade}
base_year = 1961
temp <-   combined_country %>%
  filter(year>= base_year, year<=2013) %>%
...
- Some forecasts predict pasture will decline, but these assume that livestock productivity will rise more quickly than it has over the past decades, an unlikely prospect without greater support for productivity growth from government, business and civil society.


## Pasture growth is particularly likely in tropical and humid regions with high biodiversity and carbon stocks
- Pasture area has slowly been shifting toward countries that are predominantly humid and tropical
- Graph of change in pasture area by climate
...
```
- I plan to add a figure of regional pasture projections, perhaps just from the Message-Globiom model which modelers have chosen as the "marker" model for the middle-of-the-road scenario.


## Tropical pasture expansion poses a great threat to biodiversity 

## Land conversion in tropical forests also has a large impact on climate

## Accelerated productivity growth, alongside reductions in ruminant meat and milk demand, is a key component to avoiding global pasture area expansion and the biodiversity and climate impacts this would cause.
- Refernce to above graph of pasture projections that shows high productivity estimates compared to others.
...
```{r Correlation Plot}
#RETURN HERE TO CREATE CROSS-SECTIONAL CORRELATION PLOT FOR 2013 with % change in pasture area, gdp, gdp pc, pop, m&d production, m&d consumption, m&d yield, stock)


#Corr plan  WITH PANEL DATA (year, pasture area, log(gdp), log(gdp pc), pop, m&d production, m&d consumption, m&d yield, stock)
```
...
```{r Bubble chart w/ minor-regions in a 2x2 grid based on their beef yields (one axis) and percent change in beef demand/production in past 10 years }
