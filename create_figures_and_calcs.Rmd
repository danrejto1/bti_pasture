...
p
ggsave(filename = "outputs/change_in_yield_by_income_group_v1.png", p, width = big_wdt, height = hgt)
```

### other old income analyses
...
```{r Old - Map of pasture area change 1961-1999}
...
pdf("outputs/map_pasture_change_00-16.pdf",width=8,height=8)
...
## Fig 9 and Productivity Figures
...
p <- ggplot(pd,aes(x=reorder(region, -value), y=1/value, fill=pasture_increased)) +
  geom_col()+
  labs(title = "Feed Efficiency Lower in Regions where Pasture Expanded", 
       x="", 
...
ggsave(filename = "outputs/feed_efficiency_by_region.pdf", p, width=12, height=8) 
```

## Old - Analysis of production in country where pasture grew
...
```{r Old - Percent Pasture Area Change in regions of interest}
...
## Old - Graphs of prodution by level of prpoductivity
...
p <- ggplot(pd, aes(x=year, y=tot, color=region)) +
  geom_line() +
  labs(x="", y = "Million tonnes", title = "Forecast Food Consumption", caption = "Data: FAO. 2018. The future of food and agriculture – Alternative pathways to 2050. Rome.", color = "Countries")+ 
...
#ggsave(filename = "outputs/forecast_beef_milk_consumption.pdf", plot = p, width = wdt, height = big_hgt)
```

```{r Fig 13 Bar Chart of Pasture Projections}
...
p <- combined_country %>%
  ...
  theme_hc()
p
ggsave(filename = "outputs/pasture_change_by_climate.pdf", plot = p, width = wdt , height = hgt)
...
p <- pd %>% 
  ...
  geom_smooth(method = "loess", se = F) + #show loess line but hide standard error bar
  geom_point(alpha=.2) + 
  labs(title = "Yield Growth May Be Slowing",
...
ggsave(plot=p, filename = "outputs/yield_growth_slowdown.pdf", width = 8, height = 8)
```
