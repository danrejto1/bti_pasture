...
p <- ggplot(pd,aes(x=reorder(region, -value), y=1/value, fill=pasture_increased)) +
  ...
  labs(x="", 
       ...
       caption="Source: Wirsenius et al. (2010).")+ 
  bti_theme+
...
p <- pasture_projections %>% 
  ...
  labs(x="Projection Name",
       y="Projected Pasture Change 2010-2050 (million ha)",
       caption = "Sources: Riahi et al. (2017), Searchinger et al. (2018).")+
  bti_theme+
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none", axis.line = element_line(size = .25))+
  scale_x_discrete(labels=function(x){sub("\\s", "\n", x)})+
  scale_fill_manual(values = rep("#0d4459",6), aesthetics = "fill")
 
#p
ggsave(filename = "outputs/fig_12_projected_pasture_2010_2050.pdf", plot = p)
```
...
```{r A1}
unadjusted_pasture_area <- read_csv(file = "data/raw_data/fao_country_level_land_area.csv") %>% filter(Item == "Land under perm. meadows and pastures") %>% group_by(Year) %>% summarize(tot = sum(Value, na.rm=T)*1000) #global pasture area in ha (converted from 1000 ha)

global_pasture_area <- read_csv(file = "data/raw_data/fao_global_perm_pasture_area.csv")

with_saudi <- full_data %>% group_by(year) %>% summarize(tot = sum(area, na.rm=T))

p <- ggplot() +
  geom_line(data = pasture_global,aes(x = year, y = area*ha_to_bha), color = "green") +  #convert to billion ha 
  geom_line(data = filter(with_saudi, year<2017), aes(x = year, y = tot*ha_to_bha), color = "#000000") + 
  geom_line(data = filter(hyde, year>=1900), aes(x = year, y = area/10000000), color = "red") +  #convert to billion ha 
  geom_line(data = unadjusted_pasture_area, aes(x = Year, y = tot*ha_to_bha), color = "blue") +  #convert to billion ha 
...
```{r Extra graph with historical trend and projections}
...
pasture_2050 <- pasture_projections %>% 
  filter(`Base Year`==2010) %>% #filter to just 2010 base years
  filter(str_detect(Source, "WRI")) %>% #just models of interest
...
p <- ggplot(pd, aes(x=year, y=area*ha_to_mha, color=`Data Series`))+
  geom_line()+
  #scale_y_continuous(limits = c(0, NA))+
  #bti_theme+
  scale_color_manual(values=cols)
...
p
ggsave(filename = "outputs/extra_historic_vs_projection.pdf", p)
```
