...
#for using downloaded Futura font (must download or buy futura from internet first)
library(showtext)
...
```{r Set themes}
#BTI visual theme
font_family <-"Futura"   #alternative if you dont have futura on comptuer is "Helvetica"
...
#color template
cols <- c("#0d4459", "#00a990", "#d05527", "#a33332", "#b2b2b1", "#dfb9a6", "#b381d0", "#2A2A2A", "#ecc627")
...
# vars for plot sizes
wdt <- 4.25 #default figure output width
hgt <- 4.25 #default figure output height
big_wdt <- 8.5 #width for figures with 2 facets side by side
big_hgt <- 8.5 #height for figures with 2 facets on top of one another
```
...
combined_country <- combined_country %>%
  filter(country !="Saudi Arabia") #omit because FAO omitted it in its pasture land use analysis, Ramankutty et al. discuss the FAO values as being orders of magnitude above subnational values, and because GDP data must be outlier
...
rest_of_world <-  c("Central America","Caribbean", "South-Eastern Asia","Southern Asia", "Western Asia", "Eastern Asia", "Oceania", NA, 
                    "North Macedonia", "Palestine", "C̫te d'Ivoire", "Runion")
...
combined_country[combined_country$country %in% rest_of_world,]$group <- "Rest of World"
combined_country[combined_country$country %in% rest_of_world,]$minor_group <- "NA"

combined_country <- filter(combined_country, is.na(minor_group)==F) #remove empty runion row that persisted from previous cleaning
...
# absolute change 1961-2016
pasture_change_1961_2016_abs <- (pasture_global$area[pasture_global$year==2016] - pasture_global$area[pasture_global$year==1961])*ha_to_mha
...
p <- ggplot(pasture_global) +
  ...
  labs(
    #title = paste0("Fig 1: Global Pasture Area Declined ", round(pasture_global_pct_fall_2000_16,1), "% (", round(pasture_global_abs_fall_2000_16/1000000,1), " Mha) since 2000"),  
       ...
       x = "",
       caption = "Source: FAO (2019).") +
  scale_y_continuous(limits = c(2.7, 3.00)) + 
...
In 2016, there were approximately `r pasture_change_1961_2016_abs` million hectares more pastureland than there were in 1961, an area roughly the size of Mexico
...
## Fig 6 and 7 and A3 - Productivity Gains Key to Peak Pasture
...
p <- pd %>% 
  ...
  labs(y = paste0("Annual Percent Change"),
       x = "",
       caption = "Categories represent FAO 'Bovine Meat' and 'Milk - Excluding Butter' items.\nLines derived from LOESS smoothing. Source: FAO (2019).") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_x_continuous(breaks = since_1961) +
  facet_wrap(item~.)+
  bti_theme + 
  theme(legend.position = "none", axis.line = element_line(size = .25))+
...
p <- global_indices %>%
  ...
  labs(y = "Percent Change 1961-2013",
       x = "",
       caption="Omitting Former Soviet Union. Source: FAO (2019).") +
...
```{r Fig A3 - Graph change in  meat & milk yields and stocking by group}
pd <- regional_indices %>%
    filter(year==2013) %>% #filter to just see % change from 1961-2013
  select(year, group, pct_chg_d_yld, pct_chg_m_yld, pct_chg_stocking) %>%   #select just yields and stocking to graph  
  gather(key = variable, value = "value", -year, -group) #gather variables into one column for easy graphing

pd$group <- factor(pd$group, levels = pd$group[order(pd$value[pd$variable == "pct_chg_d_yld"], decreasing = T)]) #reorder group for grpahing
  
p <- ggplot(pd, aes(x= group, y = value-1, fill = variable, group = variable))+   
    geom_bar(stat="identity", position="dodge") +
    labs(x=element_blank(),
         y="Percent Change 1961-2013",
         caption = "Omitting Former Soviet Union. Source: FAO (2019).")  +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  ...
  bti_theme +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.line = element_line(size = .25))

#p

ggsave(filename = "outputs/fig_a3_change_in_yld_by_geographic_group.pdf", p)
```
...
income_indices <- combined_country %>%
  ...
  summarize(m_yld = sum(m_prod, na.rm=T)/sum(m_head, na.rm=T),  #beef yield in  metric tons/animal
            d_yld = sum(d_prod, na.rm=T)/sum(d_head, na.rm=T),  # milk yield in  metric tons/animal
            m_cons = sum(m_cons_total, na.rm=T),  #beef consumption total in  metric tons
            d_cons = sum(d_prod, na.rm=T),  # dairy production total in  metric tons
            m_prod = sum(m_prod, na.rm=T),  #beef production total in  metric tons
            d_prod = sum(d_cons_total, na.rm=T),  # dairy consumption tot in metric tons
...
p <- ggplot(pasture_area_change_by_income) +
  ...
  labs(x="Year", y="Pasture Area (million ha)", 
       title = "", 
       caption = "Omitting Countries Missing World Bank Income Group. Source: FAO (2019).") +
  scale_x_continuous(breaks = since_1961) + 
...
p <- pasture_by_income %>% 
  ...
  labs(x="", y="Percent Change since 1961", 
       caption = "Omitting countries missing World Bank income group. Sources: FAO (2019), World Bank (2018).") +
  scale_x_continuous(breaks = since_1961) + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) + 
...
#p
ggsave(filename = "outputs/fig_3pct_change_in_pasture_by_income_group.pdf")
```
...
p <- income_indices %>% 
  ...
  filter(year<=2016) %>%
  ggplot() + geom_line(aes(x=year, y=value * t_to_kg, color = inc_group_16)) +  #convert yields from metric tons to kg
  labs(x="", y="Yields (kg product/animal)", 
       caption = "Omitting Former Soviet Union & countries missing World Bank income group.\nSources: FAO (2019), World Bank (2018).") +
  bti_theme +
...
## Fig 2 and A2 - Maps
```{r Fig A2 - Map of pasture area change by country since 1961 (in Mha)}
pdf("outputs/fig_a2_map_pasture_change_61-16.pdf",width=big_wdt,height=hgt)
...
viz <- joinCountryData2Map(dF = as.data.frame(pasture_chg_annual_00_16), #need to coerce to df or theres an error
                           joinCode="ISO3", nameJoinColumn="iso_code") #match data on ISO code. more countries in our data match when using ISO code 
...
mapParams <- mapCountryData(viz, 
                            ...
                            missingCountryCol = "grey") 
#create map
do.call(addMapLegend, c(mapParams, legendLabels="all",legendWidth=0.5)) #alternate display for legend legendIntervals = "page"
...
p <- ggplot(pd,aes(x=reorder(region, -value), y=1/value, fill=pasture_increased)) +
  ...
  labs(x="", 
       ...
       caption="Note: Values from 1992/1994\nSource: Wirsenius et al. (2010).")+ 
  bti_theme+
  theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.line = element_line(size = .25)) +  
  facet_wrap(variable~., scales = "free")+
...
#merge  and subtract to get change since baseline
...
p <- plot_dat %>% ggplot() +
  ...
  labs(title="",
       x="",
       y="Pasture Area Change Since 1961 (million ha)",
       caption = "Source: FAO (2019).") +
  scale_y_continuous(labels = point) +
  scale_x_continuous(breaks = since_1961) + 
  bti_theme+
  theme(legend.title =element_blank(),axis.line = element_line(size = .25))+
  scale_color_manual(values=cols)
...
pd$item <- recode(pd$item, "Beef and veal" = "Beef", "Raw milk" = "Milk")

p <- ggplot(pd, aes(x=year, y=tot, color=region)) +
  geom_line() +
  labs(x="", y = "Million metric tons", 
       caption = "Values are for 'Beef and veal' and 'Raw milk' FAO items.\nSource: FAO (2018).", 
       color = "Countries")+ 
  scale_color_manual(values = cols)+
  facet_wrap(item~., nrow = 1, scales = "free")+
...
ggsave(filename = "outputs/fig_10_forecast_beef_milk_consumption.pdf", plot = p)
```


```{r Prep data for projection graph}
#create df with 2010 and 2050 pasture values 
pasture_2010 <- pasture_global[pasture_global$year==2010,"area"] %>% pull()  #get 2010 pasture area
pasture_2010 <- pasture_2010*ha_to_mha #convert to million ha

pasture_2050 <- pasture_projections %>% 
  filter(str_detect(Label, c("Historical"))) %>% #filter to just single WRI model of interest
  select(Label, `Projected Absolute Change (Mha)`)

iassa_projections <- read_csv(file = "data/pasture_projections/iassa_change_from_2010_values.csv")
iassa_projections <- iassa_projections %>% 
  select(Model, c_2020, c_2030, c_2040, c_2050) %>% 
  mutate(`2010`= pasture_2010) %>%
  mutate(`2020`=`2010` + c_2020, `2030`=`2010`+c_2030, `2040`=`2010`+c_2040, `2050`=`2010`+c_2050) %>% 
  select(-c_2020, -c_2030, -c_2040, -c_2050) %>% 
  gather(key = year, value = area, -Model) %>% 
  rename("Data Series"=Model) %>% 
  mutate(year=as.numeric(year), area = area*1000000) #convert from million ha to ha
  

projection_values <- tibble(`Data Series` = pasture_2050$Label, change = pasture_2050$`Projected Absolute Change (Mha)`, `2010`= pasture_2010) %>%
  mutate(`2050` = `2010`+change) %>% 
  select(-change) %>% 
  gather(key = "year", value = "area", -`Data Series`) %>% 
  mutate(year=as.numeric(year), area = area*1000000) #convert from million ha to ha

pd <- pasture_global %>% 
  mutate(`Data Series` = "FAO (Adjusted)") %>% 
  bind_rows(projection_values, iassa_projections)
```

```{r Graph projections}
p <- ggplot(pd, aes(x=year, y=area*ha_to_bha, color=`Data Series`)) +
  geom_line() +
  labs(x = "", y = "Pasture Area (Billion Ha)", color = "Projection", 
       caption = "FAO historical pasture data (adjusted) shown in turquoise.\nSources: Riahi et al. (2017), Searchinger et al. (2018).") +
  scale_color_manual(values=cols) + 
  bti_theme
    
p
ggsave(filename = "outputs/fig_12_historic_vs_projection.pdf", p, width = 8, height = 5)
```
...
p <- combined_country %>%
  ...
  geom_line()+
    labs(title = "",
       x="",
       y="Pasture Area Change since 1961 (million ha)",
       caption = "Source: Adapted from Godde et al. (2018).",
       color="Climate")+  #change legend title
  scale_y_continuous(labels = point)+
  scale_x_continuous(breaks = since_1961)+
  scale_color_manual(values=cols, labels = climate_labeller)+
  bti_theme

#p

ggsave(filename = "outputs/fig_5_pasture_change_by_climate.pdf", plot = p)
...
p <- pd %>% filter(Country != "Oceania") %>% #plot w/o Oceania
  ...
  labs(x="", y="Ratio of Domestic Bovine Meat\n Production to Supply", 
       caption = "Omitting Oceania, which produces over 2x domestic supply.\nSource: FAO (2019).")+
  bti_theme+
  theme(legend.title = element_blank(), axis.line = element_line(size = .25))+
  scale_color_manual(values=cols)
...
## A1 Hyde, Original and Adjusted FAO Pasture Area
```{r A1}
#create dataframes with unadjusted pasture area from FAO's global values and adjusted values but with Saudi Arabia left in
unadjusted_pasture_area <- read_csv(file = "data/raw_data/fao_global_perm_pasture_area.csv") %>% 
  select(-Unit, -Domain, -Area, -Element, -Item) %>% 
  mutate(Value  = Value*1000, Source = "FAO (Unadjusted)") %>%  #convert from 1000 ha to ha & add column for sources
  rename("area"=Value, "year" = Year)

with_saudi <- full_data %>% group_by(year) %>% summarize(area = sum(area, na.rm=T)) %>% mutate(Source = "FAO (Adjusted inc. Saudi Arabia)")

#combine dataframes with Hyde and adjusted data without Saudi Arabia
combined_global_series <- hyde %>% 
  mutate(area = area*100) %>%  #convert from 100 ha to ha
  select(-region) %>% 
  mutate(Source = "HYDE v3.2") %>% 
  bind_rows(with_saudi, unadjusted_pasture_area, mutate(pasture_global, Source = "FAO (Adjusted exc. Saudi Arabia)"))
  

p <- combined_global_series %>% filter(year>=1940 & year<2017) %>% #just shiow values for 1990+
  ggplot(aes(x = year, y = area * ha_to_bha, color = Source)) +
  geom_line()+
  labs(y = "Global Pasture Area (billion ha)", 
       x = "",
       caption = "Sources: FAO (2019), Klein Goldewijk et al. (2017)") +
  bti_theme+
  scale_color_manual(values = cols)+
  scale_x_continuous(breaks = c(seq(1940,2010,10)))+
  scale_y_continuous(limits = c(NA, 3.5), breaks = c(seq(2.5,3.5,.25)), labels =  c(seq(2.5,3.5,.25)))
p
  
ggsave(filename = "outputs/fig_a1_global_pasture_area_trend_comparison.pdf", plot = p, width=10, height = 10)
```


## A4 Yield growth in high income countries may be slowing
...
```{r Fig A4 - Graph Yield Slowdown in High Income Countries}
p <- pd %>% 
  filter(inc_group_16 %in% c("H")) %>%  #specify income groups
  ggplot(aes(x=year, y=pct_chg-1, color=inc_group_16))+  #color different items
  geom_smooth(method = "lm", se = T) + #show loess line but hide standard error bar
  geom_point(alpha=.2) + 
  labs(y = ("Annual Percent Change"),
       x = "",
       caption = "Gray represents 95% confidence interval.\nSource: FAO(2019).",
       color="") +
...
p
ggsave(plot=p, filename = "outputs/fig_a4_yield_growth_slowdown.pdf") #, width = 8, height = 8
```

```{r Extra line graph of GHGs from forest to pasture considering to include in main text}
#load data on GHG emissions from land use change by country from Hougton et al.. Data provided in 2017 by Richard Houghton and Zander Nassikas, Woods Hole Reserach Center on November 14, 2017
houghton_wide <- data.table::fread(input = "data/raw_data/houghton_2017_v5_FRA2015_Ag_process_based_netflux_world_countries.csv", skip = 1)

#reshape and summarize data for FP (forest-to-pasture) and OP (other to pasture) and CP (cropland to pasture)
houghton_long <- t(houghton_wide)
houghton_long <- as_tibble(houghton_long, rownames = NA) %>%  mutate(country = names(houghton_wide))

names(houghton_long) <- houghton_long[1,]

#summarize to global level, just for forest-to-pasture
pd <- houghton_long %>% 
  gather(key = "year", value = "value",-Country, -Process, -Units) %>% 
  filter(Process %in% c("FP")) %>% 
  mutate(value = as.numeric(value), year=as.integer(year)) %>% 
  select(-Units) %>% 
  dplyr::group_by(year, Process) %>% 
  dplyr::summarize(ghg = sum(value, na.rm=T)) %>% 
  filter(year>=1960) #filter to time period we examine elsewhere in report

#plot
p <-  ggplot(pd, aes(x=year, y=ghg, color=Process)) + 
  labs(x="", y="Net Annual Emissions (GtC)",
       caption = "Source: Houghton & Nassikas (2017)") +
  geom_line() +
  #geom_abline(aes(slope=0, intercept=0), size=.25, linetype = "dashed")+  #can add for horizontal line at 0
  scale_color_manual(values=cols) +
...
ggsave(filename = "outputs/extra_ghg_from_forest_to_pasture.pdf", plot = p)
```
